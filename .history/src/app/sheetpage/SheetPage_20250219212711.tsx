...
export default function Sheet() {
  ...
  const validateHeaders = (values: Record<string, string>) => {
  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate(variables) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        // Add existing rows that aren't being updated
        currentTable.rows
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      //   console.log("Setting pending batch with updates", prev);

      //   console.log("all rows in table: ", tableData.rows);
      ...
      //   const newState = {
      //     updates: [...updatedRows], // Don't filter or merge, just set the updates
      //     newRows: prev.newRows.map((row) => ({
      //       ...row,
      //       values: {
      //         ...row.values,
      //         ...Object.fromEntries(
      //           newHeaders.map((h) => [h.id.toString(), ""]),
      //         ),
      //       },
      //     })),
      //   };

      //   console.log("New pending batch state:", newState);
      //   return newState;
      // });

      // // Queue updates for ALL existing rows
      // setPendingBatch((prev) => {
      //   console.log("Current pending batch:", prev);
      ...
      //     },
      //   }));
      //   const existingUpdates = prev.updates.filter(
      //     (update) =>
      //       !updatedRows.some((newUpdate) => newUpdate.rowId === update.rowId),
      //   );
      //   console.log("Queuing updates for rows:", updatedRows);

      //   return {
      //     updates: [...existingUpdates, ...updatedRows],
      //     newRows: prev.newRows.map((row) => ({
      //       ...row,
      //       values: {
      //         ...row.values,
      ...
      //     // Map old header IDs to new ones
      ...
      //     // Ensure all new headers are present
      //     return {
      //       ...mappedValues,
      //       ...Object.fromEntries(newHeaders.map((h) => [h.id.toString(), ""])),
      //     };
      ...
      //   // Update ALL rows, not just existing ones
      //   const allRowUpdates = [
      //     // Existing rows (positive IDs)
      //     ...tableData.rows
      //       .filter((row) => row.id > 0)
      //       .map((row) => ({
      //         rowId: row.id,
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      const validatedValues = validateHeaders(rowValues);
  ...
  const handleRowCreate = useCallback(() => {
    ...
    const validatedValues = validateHeaders(initialValues);
