...
export default function Sheet() {
  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    ...
    onSuccess(data) {
      // Ensure we update the cache with the server response
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        if (!old) return old;
        return {
          ...old,
          tables: old.tables.map((table) =>
            table.id === tableData?.id
              ? {
                  ...table,
                  rows: data?.updatedRows ?? table.rows,
                }
              : table,
          ),
        };
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      // Get the latest data including any completed mutations
      const latestData = utils.sheet.findSheet.getData({ id: sheetId });
      const currentTable = latestData?.tables.find((t) => t.id === tableId);
          ...
          tables: old.tables.map((table) =>
            table.id === tableId
              ? {
                  ...
                  // Use the latest rows from the server
    ...
    onSuccess(newHeaders, variables, ctx) {
      // Ensure we update with server state
      utils.sheet.findSheet.invalidate({ id: sheetId });
