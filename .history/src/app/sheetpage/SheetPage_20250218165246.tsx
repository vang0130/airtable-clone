...
export default function Sheet() {
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    onSuccess(newHeaders, variables, ctx) {
      ...
      setTableData((prev) => {
        ...
        const rowUpdates = tableData.rows
          .filter((row) => row.id > 0) // Only update real rows
          .map((row) => ({
            rowId: row.id,
            values: {
              ...row.values,
        ...
        setPendingBatch((prev) => ({
          ...prev,
          updates: [...prev.updates, ...rowUpdates],
        }));

        return {
          ...prev,
          headers: updatedHeaders,
          rows: rowUpdates.map((update) => ({
            id: update.rowId,
  ...
  const handleRowCreate = useCallback(() => {
    ...
    tableData.headers
      .filter((header) => header.id > 0) // Only use real headers
      .sort((a, b) => a.id - b.id)
      .forEach((header) => {
    ...
    // Create new object reference for pendingBatch
    setPendingBatch((prev) => ({
      ...prev,
      newRows: [
        ...prev.newRows,
        { tableId: tableData.id, values: { ...values }, tempId },
      ],
    }));
  ...
  // Add useRef for timeout
  const batchTimeoutRef = useRef<NodeJS.Timeout>();

  // Batch processing effect with proper timing
  useEffect(() => {
    ...
    batchTimeoutRef.current = setTimeout(() => {
      void (async () => {
        ...
        const timeSinceLastHeader = Date.now() - lastHeaderAddTime.current;
        const timeSinceLastCell = Date.now() - lastActiveCellTime.current;

        try {
          processingRef.current = true;
          console.log("Checking batch processing timings:", {
            timeSinceLastHeader,
            timeSinceLastCell,
            pendingHeaders,
            pendingBatch,
            activeCell, // Add this to debug
          });

          console.log("Pending headers:", pendingHeaders);
          console.log("Pending batch:", pendingBatch);
          console.log("timesince last header", timeSinceLastHeader);
          console.log("timesince last cell", timeSinceLastCell);
          console.log("object keys", Object.keys(pendingHeaders));
          console.log("object values", Object.values(pendingHeaders));
          // Process headers only if no new headers for 3 seconds
          if (
            timeSinceLastHeader > 1000 &&
            pendingHeaders &&
            Object.values(pendingHeaders).length > 0
          ) {
            console.log("Processing headers after delay", {
              pendingHeaders,
              timeSinceLastHeader,
            });
            ...
            if (newHeaders.length > 0) {
              ...
              // Queue row updates for new columns
              if (tableData?.rows) {
                const rowUpdates = tableData.rows
                  .filter((row) => row.id > 0)
                  .map((row) => ({
          ...
          // Process row updates only if no cell activity for 2 seconds
          if (
            timeSinceLastCell > 2000 &&
            (pendingBatch.updates.length > 0 || pendingBatch.newRows.length > 0)
          ) {
            console.log("Processing rows after cell inactivity");
