...
export default function Sheet() {
  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate({ updates, newRows }) {
              ...
              ? {
                  ...
                  rows: table.rows
                    .map((row) => {
                      ...
                      return update
                        ? { ...row, values: update.values as JsonValue }
                        : row;
                    })
                    .concat(
                      newRows.map((newRow) => ({
                        ...
                        values: newRow.values as JsonValue,
                        createdAt: now,
                        sheetId: sheetId,
      ...
      return { previousData, preservedHeaders: currentHeaders };
    },

    onSuccess(data, variables, context) {
      // Ensure we maintain headers when updating with new row data
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        if (!old) return old;
        return {
          ...old,
          tables: old.tables.map((table) =>
            table.id === tableData?.id
              ? {
                  ...table,
                  headers: context?.preservedHeaders ?? table.headers,
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      return {
        prevData,
        headersWithTempIds: headersWithIds,
        preservedRows: currentRows,
      };
    },
          ...
          tables: old.tables.map((table) =>
            table.id === tableData?.id
              ? {
                  ...
                  rows: ctx.preservedRows, // Maintain existing rows
                }
              : table,
          ),
        };
