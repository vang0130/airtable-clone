...
export default function Sheet() {
  ...
  const handleHeaderAdd = async () => {
    ...
    // Immediate UI update with temp ID - preserve existing values
    setTableData((prev) => {
      console.log("Setting temp header ID:", {
        tempId,
      ...
      return {
        ...
        rows: prev!.rows.map((row) => ({
          ...row,
          values: {
            ...validateHeaders(row.values), // Preserve existing values
            [tempId.toString()]: "", // Only add the new column
          },
        })),
      };
    });
    ...
    try {
      ...
      const createdHeaders = await addHeader.mutateAsync({
        ...
        headers: [{ name: headerInput.trim() }],
      });

      if (createdHeaders?.[0]) {
        setTableData((prev) => {
          const updatedHeaders = prev!.headers.map((h) =>
            h.id === tempId && createdHeaders[0]
              ? { id: createdHeaders[0].id, name: createdHeaders[0].name }
              : h,
          );
          ...
          return {
            ...
            rows: prev!.rows.map((row) => ({
              ...
              values: Object.entries(row.values).reduce((acc, [key, value]) => {
                const realId =
                  key === tempId.toString()
                    ? (createdHeaders[0]?.id.toString() ?? key)
                    : key;
                acc[realId] = value;
                return acc;
              }, validateHeaders(row.values)), // Preserve all values when mapping IDs
            })),
          };
        });
      }
