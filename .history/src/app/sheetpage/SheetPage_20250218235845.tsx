  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        console.log("Header ID mapping:", Object.fromEntries(headerIdMap));

        const updatedTable = {
          ...currentTable,
          // Use new headers directly from server
          headers: newHeaders,
          // Update rows, replacing temp IDs with real IDs
          rows: currentTable.rows.map((row) => ({
            ...row,
            values: {
              // Start with existing values
              ...(row.values as Record<string, string>),
              // Add new headers with empty values
              ...Object.fromEntries(
                newHeaders.map((h) => [h.id.toString(), ""]),
              ),
              // Map any temp IDs to real IDs
              ...Object.fromEntries(
