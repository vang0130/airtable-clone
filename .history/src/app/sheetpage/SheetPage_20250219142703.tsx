          ...
          tables: old.tables.map((table) =>
            table.id === tableData?.id
              ? {
                  ...table,
                  rows: [
                    // Only include rows that aren't being updated
                    ...table.rows.filter(
                      (row) =>
                        !updates.some((u) => u.rowId === row.id) && row.id > 0, // Only keep existing rows, not temp ones
                    ),
                    // Add updated rows
                    ...updates.map((update) => ({
                      ...table.rows.find((r) => r.id === update.rowId)!,
                      values: update.values,
                    })),
                    // Add new rows only if they weren't added by header mutation
                    ...newRows
                      .filter((newRow) => !table.rows.some((r) => r.id < 0))
                      .map((newRow, index) => ({
                        id: -(Date.now() + index),
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        console.log(
          "Current rows:",
          currentTable.rows.map((r) => r.id),
        );
        console.log(
          "Updates for rows:",
          variables.updates.map((u) => u.rowId),
        );
        console.log("New rows to add:", variables.newRows.length);

        // Create a Map for unique rows
        const uniqueRows = new Map();

        // First add existing rows that aren't being updated
        currentTable.rows
          .filter(
            (row) =>
              row.id > 0 && !variables.updates.some((u) => u.rowId === row.id),
          )
          .forEach((row) => uniqueRows.set(row.id, row));

        // Add updated rows
        variables.updates.forEach(
          (update: { rowId: number; values: Record<string, string> }) => {
        ...
        // Add new rows only if no temp rows exist
        if (!currentTable.rows.some((r) => r.id < 0)) {
          variables.newRows.forEach((newRow, index) => {
            const tempId = -(Date.now() + index);
            uniqueRows.set(tempId, {
              id: tempId,
              tableId: newRow.tableId,
              values: newRow.values,
              createdAt: new Date(),
        ...
        console.log("Final unique row IDs:", Array.from(uniqueRows.keys()));
      ...
      setPendingBatch((prev) => {
        const updatedValues = (values: Record<string, string>) => {
          const mappedValues = Object.fromEntries(
            Object.entries(values)
              .map(([key, value]) => {
                ...
                if (realId) {
                  console.log(
                    `Mapping batch value from temp ID ${key} to real ID ${realId}`,
                  );
                }
                return realId ? [realId, value] : [key, value];
              })
          ...
          console.log("Final batch values:", {
            final: finalValues,
            finalTempIds: Object.keys(finalValues).filter(
              (id) => Number(id) < 0,
            ),
          });
        ...
        const existingRowUpdates = [
          ...variables.updates.map(
            (update: { rowId: number; values: Record<string, string> }) => ({
              rowId: update.rowId,
        ...
        const result = {
          ...
          newRows: variables.newRows.map((row) => ({
            ...row,
            values: updatedValues(row.values),
          })),
        };
