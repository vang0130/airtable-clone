...
export default function Sheet() {
  ...
  // check that all columns are present in every row
  const validateHeaders = useCallback(
    // takes in headers as input
    (values: Record<string, string>) => {
      // if no headers
      ...
      return {
        ...
        ...Object.fromEntries(
          tableData.headers.map((h) => [
            h.id.toString(),
            // either current value or empty string
      ...
      void (async () => {
        ...
        try {
          ...
          if (pendingHeadersEntries.length > 0) {
            ...
            console.log("New headers to process:", newHeaders);

            if (newHeaders.length > 0) {
              const createdHeaders = await addHeader.mutateAsync({
                tableId: headerData.tableId,
                headers: newHeaders.map((h: { name: string }) => ({
                  name: h.name,
                })),
              });
            ...
            if (
              ...
              // Ensure all values have proper headers before sending
              const processedUpdates = pendingBatch.updates.map((update) => {
                const validated = validateHeaders(update.values);
                console.log(`Validated update for row ${update.rowId}:`, {
              ...
              const processedNewRows = pendingBatch.newRows.map((row) => {
                const validated = validateHeaders(row.values);
                console.log(`Validated new row ${row.tempId}:`, {
                  before: row.values,
                  after: validated,
                });
                return {
          ...
          console.error("Error processing updates:", {
            error,
            pendingHeaders,
            pendingBatch,
          });
