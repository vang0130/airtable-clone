...
export default function Sheet() {
            ...
            table.id === tableData?.id
              ? {
                  ...
                  rows: [
                    ...
                    // Add updated rows with preserved positions
                    ...variables.updates
                      .map((update) => {
                    ...
                    // Add new rows with calculated positions
                    ...variables.newRows.map((newRow, index) => {
                      const maxPosition = Math.max(
                        ...table.rows.map((r) => r.position ?? 0),
                        0,
                      );
                      return {
                        id: newRow.id,
  ...
  const handleHeaderAdd = (newHeader: { name: string }) => {
    ...
    const maxPosition = Math.max(
      ...tableData.headers.map((h) => h.position),
      ...(pendingChanges.headers[tableData.id]?.headers.map(
        (h) => h.position,
      ) ?? []),
      0,
    );
    ...
    // Update existing non-pending rows
    const existingRows = tableData.rows.filter((row) => !row.isPending);
    ...
    const updatedNewRows = pendingChanges.rows.newRows.map((row) => ({
      ...row,
      values: {
        ...row.values,
        [nextPosition.toString()]: "",
  ...
  const columns = useMemo(() => {
    ...
    console.log("Headers before sort:", tableData.headers);

    return tableData.headers
      .slice() // Create a copy to avoid mutating original array
      .sort((a, b) => a.position - b.position) // Sort strictly by position
      .map((header) => {
        return columnHelper.accessor(
          (row) => {
            // Use position as the key to access values
            const value = row.values[header.position.toString()];
            return value ?? "";
          },
          {
            id: header.position.toString(), // Use position as stable identifier
            header: () => header.name,
            cell: (info) => (
              <Cell
                info={info}
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Calculate next position and ID
    const maxPosition = Math.max(
