...
export default function Sheet() {
  ...
  const handleHeaderAdd = (newHeader: { id: number; name: string }) => {
    console.log("Adding header:", newHeader);
      ...
      headers: {
        [tableData.id]: {
          ...
          headers: [
            ...
            { id: newHeader.id, name: newHeader.name },
          ],
        },
      },
      rows: prev.rows, // Preserve existing row changes
    }));
    ...
    console.log("Pending changes after header addition:", pendingChanges);
  };
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      console.log("Cell Update Started:", { rowId, headerId, value });
      ...
      const updatedValues = {
        ...
        ...Object.fromEntries(
          allHeaders.map((header, index) => {
            const headerKey = (index + 1).toString();
            return [headerKey, rowValues[headerKey] ?? ""];
          }),
        ),
      };
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Create row with all headers, using sequential IDs
    const newRowValues = Object.fromEntries(
      allHeaders.map((header, index) => [
        (index + 1).toString(), // Sequential IDs starting from 1
        "",
      ]),
    );
    ...
    console.log("Pending changes after row creation:", pendingChanges);
  }, [tableData?.id, tableData?.headers, pendingChanges.headers]);
