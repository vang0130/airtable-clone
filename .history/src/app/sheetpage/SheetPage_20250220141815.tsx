...
export default function Sheet() {
  ...
  const handleRowCreate = useCallback(() => {
    if (processingHeaders || headerProcessingRef.current) {
      console.log(
        "Waiting for headers to finish processing before creating new row...",
      );
      return;
    }

    if (!tableData?.id) {
      console.log("Waiting for table to finish processing...");
      return;
    }
  ...
  // HEADER MUTATIONS
  useEffect(() => {
    // do not proceed if row mutations are happening or if header already mutating
    if (processingRef.current || headerProcessingRef.current) return;
  ...
  useEffect(() => {
    ...
    // do not proceed if row processing is happening
    if (activeCell || headerProcessingRef.current || processingRef.current)
      return;
