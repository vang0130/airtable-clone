              ...
              table.id === tableData?.id
                ? {
                    ...
                    rows: table.rows.map((row) => {
                      const updatedRow = data.updatedRows.find(u => u.id === row.id);
                      return updatedRow ? { ...row, values: updatedRow.values } : row;
                    }),
                  }
                : table,
          ),
    ...
    const timeoutId = setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          const hasUpdates = pendingBatch.updates.length > 0 || pendingBatch.newRows.length > 0;
                ...
                return {
                  ...
                  tables: old.tables.map(table =>
                    table.id === tableId
                      ? {
                          ...table,
                          headers: [...table.headers, ...newHeaders.map(h => ({
                            ...h,
                            createdAt: new Date(),
                            tableId
                          }))],
