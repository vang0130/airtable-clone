...
export default function Sheet() {
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      const validatedValues = validateHeaders(updatedValues);
      ...
      setTableData((prev) => {
        ...
        return {
          ...prev,
          rows: prev.rows.map((row) =>
            row.id === rowId ? { ...row, values: validatedValues } : row,
          ),
        };
      });
      ...
      setPendingBatch((prev) => {
        if (rowId > 0) {
          ...
          return {
            ...prev,
            updates: [...otherUpdates, { rowId, values: validatedValues }],
          };
        } else {
          return {
            ...prev,
            newRows: prev.newRows.map((row) =>
              row.tempId === rowId ? { ...row, values: validatedValues } : row,
            ),
          };
        }
      });
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Start with empty values for new row
    ...
    const rowsNeedingUpdate = tableData.rows
      ...
      .map((row) => {
        // For each row that needs updating, only add the headers it's missing
        const existingValues = row.values;
        const updatedValues = {
          ...existingValues,
          ...Object.fromEntries(
            tableData.headers
              .filter((header) => !(header.id.toString() in existingValues))
    ...
    console.log("Rows needing updates:", {
      rowsNeedingUpdate,
      missingHeadersPerRow: rowsNeedingUpdate.map((row) => ({
        rowId: row.rowId,
        missingHeaders: Object.keys(row.values).filter(
          (k) => !(k in tableData.rows.find((r) => r.id === row.rowId)?.values),
        ),
      })),
    });
  ...
  }, [tableData?.id, tableData?.headers, tableData?.rows]);
    ...
    const timeoutId = setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          if (pendingHeadersEntries.length > 0) {
            console.log("Processing headers:", pendingHeaders);
            const entry = pendingHeadersEntries[0];
            const [tableIdStr, headerData] = entry as [
              string,
              { tableId: number; headers: { id: number; name: string }[] },
            ];
            const newHeaders = headerData.headers.filter((h) => h.id < 0);
            ...
            if (newHeaders.length > 0) {
              try {
                ...
                // Wait for header update to complete
                await new Promise((resolve) => setTimeout(resolve, 500));
              } catch (headerError) {
                console.error("Error creating headers:", headerError);
                return; // Exit early if header creation fails
              }
            }
          }

          // Process any pending row updates or new rows
          if (
            ...
            try {
              const allRowUpdates = [
                // Only include existing rows (positive IDs)
                ...(tableData?.rows
                  ?.filter((row) => row.id > 0)
                  .map((row) => ({
                    rowId: row.id,
                ...
                // Only include updates for existing rows
                ...pendingBatch.updates.filter((update) => update.rowId > 0),
                // Don't include temp rows in updates array
              ];

              const result = await batchUpdate.mutateAsync({
                updates: allRowUpdates,
                newRows: pendingBatch.newRows, // These will be created, not updated
              });
              console.log("Batch update result:", result);
              setPendingBatch({ updates: [], newRows: [] });
            } catch (rowError) {
              console.error("Error updating rows:", rowError);
            }
          }
        } catch (error) {
