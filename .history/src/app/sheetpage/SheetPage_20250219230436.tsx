...
export default function Sheet() {
  ...
  }, [tableData?.id, tableData?.headers, tableData?.rows, processingHeaders]);
            ...
            try {
              ...
              if (newHeaders.length > 0) {
                try {
                  ...
                  const idMapping = newHeaders.reduce(
                    (acc, temp, index) => {
                      acc[temp.id] = createdHeaders[index]?.id ?? temp.id;
                      return acc;
                    },
                    {} as Record<number, number>,
                  );

                  // Update all rows with real header IDs
                  setTableData((prev) => ({
                    ...prev!,
                    headers: prev!.headers.map((h) =>
                      h.id < 0
                        ? createdHeaders.find((ch) => ch.name === h.name)!
                        : h,
                    ),
                    rows: prev!.rows.map((row) => ({
                      ...row,
                      values: Object.entries(row.values).reduce(
                        (acc, [key, value]) => {
                          ...
                          acc[idMapping[tempId]?.toString() ?? key] = value;
                          return acc;
                        },
          ...
          if (
            ...
            try {
              const result = await batchUpdate.mutateAsync({
                // Only include updates for existing rows (positive IDs)
                updates: pendingBatch.updates.filter(
                  (update) => update.rowId > 0,
                ),
                // Keep newRows as is
                newRows: pendingBatch.newRows,
              });
