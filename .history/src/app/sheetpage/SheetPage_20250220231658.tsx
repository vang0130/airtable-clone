...
export default function Sheet() {
  ...
  const saveAllChanges = useCallback(async () => {
    ...
    console.log("Starting save with changes:", {
      pendingChanges,
      headerChanges: Object.keys(pendingChanges.headers).length,
      updateChanges: pendingChanges.rows.updates.length,
      newRowChanges: pendingChanges.rows.newRows.length,
      fullRows: pendingChanges.rows,
    });
    ...
    try {
      console.log("Starting save with changes:", {
        headers: pendingChanges.headers,
        ...
        if (headerData) {
          ...
          headerIdMapping = Object.fromEntries(
            headerData.headers.map((h, index) => [
              h.id.toString(),
              parseInt(createdHeaders[index]?.id.toString() ?? "0"),
            ]),
  ...
  const handleHeaderAdd = (newHeader: { name: string }) => {
    ...
    console.log("Pending changes after header add 2:", pendingChanges);
  };
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // Get all headers including pending ones
      const allHeaders = [
        ...(tableData?.headers ?? []),
        ...(pendingChanges.headers[tableData.id]?.headers ?? []),
      ];

      // Create a complete values object with placeholders for all headers
      const completeValues = {
        // First create empty values for all headers (including pending ones)
        ...Object.fromEntries(
          allHeaders.map((header) => [header.id.toString(), ""]),
        ),
        // Then overlay existing values
        ...rowValues,
