...
export default function Sheet() {
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    onSuccess(newHeaders, variables, ctx) {
      if (!newHeaders?.length || !tableData?.id || !ctx?.headersWithTempIds)
        return;
      pendingHeadersRef.current = pendingHeadersRef.current.filter(
        (h) => !ctx?.headersWithTempIds?.find((th) => th.id === h.id),
      );
      // Single cache update with real IDs
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        if (!old) return old;
        return {
          ...old,
  ...
  const pendingHeadersRef = useRef<Array<{ id: number; name: string }>>([]);
  const pendingRowsRef = useRef<
    Array<{ id: number; values: Record<string, string> }>
  >([]);
  ...
  const handleHeaderAdd = () => {
    ...
    pendingHeadersRef.current = [...pendingHeadersRef.current, newHeader];
    ...
    // Update any pending rows with the new header
    pendingRowsRef.current = pendingRowsRef.current.map((row) => ({
      ...row,
      values: {
        ...row.values,
        [tempId]: "",
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Include both real headers and ALL pending headers
    [...tableData.headers, ...pendingHeadersRef.current].forEach((header) => {
      values[header.id.toString()] = "";
    });

    const newRow = { id: tempId, values };
    pendingRowsRef.current = [...pendingRowsRef.current, newRow];
                ...
                const updatedValues = (values: Record<string, string>) => ({
                  ...
                  ...Object.fromEntries(
                    Object.entries(values)
                      .filter(([key]) => {
                        const tempHeader = newHeaders.find(
                          (h) => h.id.toString() === key,
                        );
                        return tempHeader !== undefined;
                      })
                      .map(([key, value]) => {
                        const tempHeader = newHeaders.find(
                          (h) => h.id.toString() === key,
                        ...
                        const realHeader =
                          tempHeader &&
                          createdHeaders.find(
                            (ch, idx) => newHeaders[idx]?.id === tempHeader.id,
                          );
                        return [
                          realHeader ? realHeader.id.toString() : key,
                          value,
