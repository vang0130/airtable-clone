...
export default function Sheet() {
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // Use the existing header ID, don't create a new one
      const updatedValues = {
        ...rowValues,
        [headerId.toString()]: value, // Use the existing header ID
      };
  ...
  // Add or modify useEffect to handle batched mutations
  useEffect(() => {
    if (activeCell) return;

    const timer = setTimeout(() => {
      // Process pending headers first
      Object.entries(pendingHeaders).forEach(([_, { tableId, headers }]) => {
        ...
        if (newHeaders.length > 0) {
          addHeader.mutate(
            ...
            {
              onSuccess: (createdHeaders) => {
                // Update all rows with the new header IDs
                const headerUpdates =
                ...
                setPendingBatch((prev) => ({
                  updates: [...prev.updates, ...headerUpdates],
      ...
      // Then process all row updates together
      if (pendingBatch.updates.length > 0 || pendingBatch.newRows.length > 0) {
        // Ensure all IDs are small negative numbers
        const sanitizedUpdates = pendingBatch.updates.map((update) => ({
          ...update,
          rowId:
            update.rowId > 0
              ? update.rowId
              : -(Math.abs(update.rowId) % 100000),
        }));

        const sanitizedNewRows = pendingBatch.newRows.map((row, index) => ({
          ...row,
          tempId: -(index + 1),
        }));
  ...
  const handleRowCreate = useCallback(() => {
    ...
    const tempId = -Date.now(); // use timestamp for consistent ordering
    ...
    setTableData((prev) => {
      ...
      return {
        ...prev,
        rows: [...prev.rows, { id: tempId, values, tableId: tableData.id }],
      };
    });
