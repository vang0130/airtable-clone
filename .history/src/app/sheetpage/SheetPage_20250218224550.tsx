    ...
    onSuccess(newHeaders, variables, ctx) {
      ...
      setPendingBatch((prev) => {
        const updatedValues = (values: Record<string, string>) => ({
          ...values,
          ...Object.fromEntries(
            tableData?.headers.map((h) => [h.id.toString(), ""]),
          ),
          ...Object.fromEntries(
            Object.entries(values)
              ...
              .map(([key, value]) => {
                const tempHeader = tableData?.headers.find(
                  (h) => h.id.toString() === key,
                );
                const realHeader =
                  tempHeader &&
        ...
        const existingRowUpdates =
          tableData?.rows
            .filter((row) => row.id > 0) // Only existing rows
            .map((row) => ({
              rowId: row.id,
        ...
        return {
          updates: [
            ...
            ...existingRowUpdates, // Add updates for all existing rows
          ],
