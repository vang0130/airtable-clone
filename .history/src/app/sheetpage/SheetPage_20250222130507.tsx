...
export default function Sheet() {
  ...
  const Cell = ({
    ...
    }, [info.getValue(), info.row.original.rowPosition, headers.headerPosition]);
    ...
    setPendingChanges((prev) => ({
      ...
      rows: {
        updates: [
          ...
          const rowA = tableData.rows.find((r) => r.position === a.position);
          const rowB = tableData.rows.find((r) => r.position === b.rowPosition);
  ...
  const columns = useMemo(() => {
    ...
    return allHeaders.map((header) => {
      return columnHelper.accessor(
        ...
        {
          headerPosition: header.position.toString(),
          header: () => header.name,
          cell: (info) => (
            <Cell
              info={info}
