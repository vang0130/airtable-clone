...
export default function Sheet() {
  ...
  const Cell = ({
    ...
    console.log("Cell render:", {
      ...
      allHeaders: tableData?.headers.map((h) => ({ id: h.id, name: h.name })),
    });

    // Verify header is current
    const currentHeader = tableData?.headers.find((h) => h.id === headers.id);
    if (!currentHeader) {
      console.warn("Cell rendering with outdated header:", {
        cellHeaderId: headers.id,
        availableHeaderIds: tableData?.headers.map((h) => h.id),
      });
    }

    // Create a unique key for each cell using both row and header IDs (temp or real)
    const cellId = `cell-${info.row.original.id}-${headers.id}`;
    const [value, setValue] = useState(info.getValue() ?? "");

    // Update local state when cell value changes externally
    useEffect(() => {
      setValue(info.getValue() ?? "");
    }, [info.getValue(), cellId]);

    return (
      <input
        key={cellId} // Add key to ensure React creates new instances
        id={cellId}
        name={cellId}
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      // Use the existing temp IDs from pendingHeaders instead of generating new ones
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      console.log("Header creation response:", {
        createdHeaders: createdHeaders.map((h) => ({ id: h.id, name: h.name })),
        pendingHeaders: pendingHeaders[tableData.id]?.headers.map((h) => ({
          id: h.id,
          name: h.name,
        })),
      });
      ...
      setTableData((prev) => {
        ...
        // Verify the arrays match in length and content
        console.log("Header arrays to map:", {
          processedHeadersLength: processedHeaders.length,
          createdHeadersLength: createdHeaders.length,
          processedHeaderNames: processedHeaders.map((h) => h.name),
          createdHeaderNames: createdHeaders.map((h) => h.name),
        });
        ...
        const idMapping = Object.fromEntries(
          processedHeaders.map((ph, idx) => {
            const mapping = [
              ph.id.toString(),
              createdHeaders[idx]?.id.toString(),
            ];
        ...
        console.log("Final mapping result:", {
          originalHeaders: prev.headers.map((h) => h.id),
          newHeaders: updatedHeaders.map((h) => h.id),
          sampleRowBefore: prev.rows[0]?.values,
          sampleRowAfter: updatedRows[0]?.values,
        });
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      console.log("Updated values prepared:", {
        rowId,
        updatedValues,
        addedHeaders: tableData?.headers
          .filter((header) => !(header.id.toString() in rowValues))
          .map((h) => h.id),
      });
      ...
      const rowsNeedingUpdate = tableData?.rows
        .filter((row) => {
          ...
          console.log("Checking row for updates:", {
            rowId: row.id,
            needsUpdate,
            missingHeaders: tableData?.headers
    ...
    return tableData.headers.map((header) => {
      ...
      return columnHelper.accessor(
        (row) => {
          ...
          console.log("Accessing cell value:", {
            rowId: row.id,
            headerId: header.id,
            value,
