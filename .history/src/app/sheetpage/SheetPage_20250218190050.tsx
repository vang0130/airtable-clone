      ...
      void (async () => {
        ...
        try {
          ...
          if (hasHeaders) {
                ...
                updates: prev.updates.map(update => ({
                  ...
                  values: Object.fromEntries(
                    Object.entries(update.values).map(([key, value]) => {
                      const realHeader = createdHeaders.find((h, idx) => 
                        newHeaders[idx]?.id.toString() === key
                      );
            ...
            const newHeaders = headers.filter((h) => h.id < 0);
            if (newHeaders.length > 0) {
              // Optimistic update for both headers and rows
              utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
                if (!old) return old;
                return {
                  ...old,
          ...
          if (hasUpdates) {
            // console.log("Processing row updates", {
            //   beforeFilter: {
            ...
            const validUpdates = pendingBatch.updates
              .filter((update) => {
                const allPositive = Object.keys(update.values).every(
                  (k) => Number(k) > 0,
                );
                console.log("Update validation:", {
                  rowId: update.rowId,
                  headerIds: Object.keys(update.values),
                  allPositive,
                });
                return allPositive;
              })
            ...
            const validNewRows = pendingBatch.newRows
              .filter((row) => {
                const allPositive = Object.keys(row.values).every(
                  (k) => Number(k) > 0,
                );
                console.log("New row validation:", {
                  tempId: row.tempId,
                  headerIds: Object.keys(row.values),
                  allPositive,
                });
                return allPositive;
              })
            ...
            if (validUpdates.length > 0 || validNewRows.length > 0) {
              ...
              console.log("No valid updates found after filtering");
            }
          }
        } catch (error) {
