...
export default function Sheet() {
  ...
  const Cell = ({
    ...
    useEffect(() => {
      if (
        ...
        document.getElementById(`cell-${info.row.original.id}-${headers.id}`)
      ) {
        setValue(info.getValue() ?? "");
      }
    }, [info.getValue(), info.row.original.id, headers.id]);
  ...
  useEffect(() => {
    ...
    console.log("useeffect for table data");
  ...
  useEffect(() => {
    ...
    if (isMutating.current) {
      console.log("useeffect for table data MUTATISENTRISENTRSIENT");
      return;
    }
      ...
      ...foundTable.headers.filter(
        (header) =>
          !pendingChanges.headers[selectedTableId]?.headers.some(
            (newHeader) => newHeader.position === header.position,
          ),
      ),
    ...
    async onMutate(variables) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        const updatedRows = tableData?.rows
          .filter((row) => !variables.newRows.some((nr) => nr.id === row.id))
          .map((row) => {
        ...
        ].sort((a, b) => a.position - b.position) as TableRow[];
        ...
        return {
          ...old,
          tables: old.tables.map((table) =>
            table.id === tableData?.id ? { ...table, rows: finalRows } : table,
          ),
        };
      });
  ...
  const updateRowsForNewHeader = (headerPosition: number) => {
    ...
    // separate existing rows from pending rows
    const existingRows = tableData.rows.filter(
      (row) =>
        !pendingChanges.rows.newRows.some((newRow) => newRow.id === row.id),
    );

    // update existing rows with new header
    ...
    // update pending rows with new header
      ...
      rows: {
        updates: [
          ...prev.rows.updates.filter(
            (u) => !updatedExistingRows.some((r) => r.rowId === u.rowId),
          ),
          ...updatedExistingRows,
        ],
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Get all current rows (both existing and pending)
    const allCurrentRows = [...tableData.rows, ...pendingChanges.rows.newRows];

    // Explicitly sort by position
    const sortedCurrentRows = allCurrentRows.sort(
      (a, b) => a.position - b.position,
    );
    console.log("Sorted current rows:", sortedCurrentRows);

    const maxPosition = Math.max(
      ...sortedCurrentRows.map((row) => row.position),
      0,
    );
    const nextPosition = maxPosition + 1;
    ...
    // Create new row with next sequential position
    const newRow = {
      id: Date.now(),
