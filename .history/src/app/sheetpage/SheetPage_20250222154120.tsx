...
export default function Sheet() {
  ...
  const updateRowsForNewHeader = (headerPosition: number) => {
    ...
    const currentRows = {
      ...
      existing: tableData.rows.filter(
        (row) =>
          ...
          !pendingChanges.rows.updates.some(
            (u) => u.rowPosition === row.rowPosition,
          ) &&
      ...
      // Currently pending updates (identified by position)
      updates: pendingChanges.rows.updates.filter(
        (update) => update.tableId === tableData.id,
      ),
