...
// When sending to server, omit tempId
type BatchUpdateRequest = {
  updates: { rowId: number; values: Record<string, string> }[];
  newRows: { tableId: number; values: Record<string, string> }[];
};
...
export default function Sheet() {
  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate(variables: BatchUpdateVariables) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        // Only add new rows if they don't exist in the table
    ...
    async onSuccess(data, variables) {
      const serverRequest: BatchUpdateRequest = {
        updates: variables.updates,
        newRows: variables.newRows.map(({ tableId, values }) => ({
          tableId,
          values,
        })),
      };
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        // First add server rows (positive IDs)
        currentTable.rows
          .filter((row) => row.id > 0)
          .forEach((row) => uniqueRows.set(row.id, row));

        // Then add temp rows (negative IDs)
        currentTable.rows
          .filter((row) => row.id < 0)
          .forEach((row) => uniqueRows.set(row.id, row));
        ...
        const updatedTable = {
          ...currentTable,
          headers: newHeaders,
          rows: Array.from(uniqueRows.values()).map((row) => ({
            ...row,
            values: {
              ...Object.fromEntries(
      ...
      setPendingBatch((prev) => {
        ...
        const existingRowUpdates = [
          ...
          // Add updates for pending new rows
