  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      setTableData((prev) => {
        ...
        // Match headers by name to ensure correct mapping
        const idMapping = Object.fromEntries(
          processedHeaders.map((ph) => {
            // Find matching header by name
            const matchingHeader = createdHeaders.find(
              (ch) => ch.name === ph.name,
            );
            const mapping = [
              ph.id.toString(),
              matchingHeader?.id.toString() ?? ph.id.toString(),
            ];
            ...
            console.log("Creating mapping:", {
              ...
              realId: mapping[1],
              headerName: ph.name,
              matchingHeaderName: matchingHeader?.name,
            });
