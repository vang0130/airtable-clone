...
export default function Sheet() {
  ...
  // Single ref for tracking all latest cell values
  const latestCellUpdatesRef = useRef<Map<string, string>>(new Map());

  // Simplified batchUpdate mutation
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate(variables) {
      ...
      // Apply all latest values in optimistic update
                    ...
                    const rowUpdates = Array.from(
                      ...
                      .reduce(
                        (acc, [key, value]) => {
                          ...
                          if (headerId) acc[headerId] = value;
                          return acc;
                        },
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      setTableData((prev) => {
        ...
        const pendingUpdates = latestCellUpdatesRef.current;
        latestCellUpdatesRef.current = new Map(); // Clear the ref
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // Always store the latest value immediately
  ...
  // Simplified row mutation effect
  useEffect(() => {
    if (pendingBatch.updates.length === 0 && pendingBatch.newRows.length === 0)
      return;
    if (processingRef.current || headerProcessingRef.current || activeCell)
      return;
    ...
    const timeoutId = setTimeout(() => void processBatch(), 2000);
    return () => clearTimeout(timeoutId);
