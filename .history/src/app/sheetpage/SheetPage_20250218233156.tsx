  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        console.log("Mapping temp to real headers:");
        ctx.headersWithTempIds.forEach((temp, idx) => {
          console.log(`Temp ID ${temp.id} -> Real ID ${newHeaders[idx]?.id}`);
        });

        // Map temp header IDs to real header IDs
        const headerIdMap = new Map(
          ctx.headersWithTempIds.map((tempHeader, idx) => [
            tempHeader.id.toString(),
            newHeaders[idx]?.id.toString(),
          ]),
        );
        ...
        const updatedTable = {
          ...
          headers: currentTable.headers.map((header) => {
            ...
            console.log(
              `Header ${header.id}: matching new header:`,
              matchingNewHeader,
            );
          ...
          // Update row values with real header IDs
      ...
      setPendingBatch((prev) => {
        const updatedValues = (values: Record<string, string>) => {
          // Get ALL headers (both old and new) that should exist in each row
          const allHeaderIds = tableData.headers
            .map((h) => h.id.toString())
          ...
          // Add empty columns for ANY missing headers (old or new)
          allHeaderIds.forEach((headerId) => {
            if (!newValues[headerId]) {
              newValues[headerId] = "";
            }
          });

          // Map any temp header IDs to real ones (for newly added headers)
          Object.entries(values).forEach(([key, value]) => {
            ...
            if (realHeader) {
              newValues[realHeader.id.toString()] = value;
              delete newValues[key];
            }
          });

          // Debug log to verify all headers are present
          console.log("All header IDs:", allHeaderIds);
          console.log("Row values:", Object.keys(newValues).sort());
          console.log(
            "Missing headers:",
            allHeaderIds.filter((id) => !newValues[id]),
          );

          return Object.fromEntries(
            Object.entries(newValues).sort(([a], [b]) => Number(a) - Number(b)),
          );
        };
