...
export default function Sheet() {
  ...
  const Cell = ({
    ...
    // Add this useEffect to maintain the value while typing
    useEffect(() => {
      if (
  ...
  // nav to correct url
  useEffect(() => {
    // Skip if mutating or if we already have a selected table

    if (isMutating.current || selectedTableId) return;
    console.log("useEffect for table data q");

    // Only navigate if we have tables but no table selected
