...
export default function Sheet() {
  ...
  const saveAllChanges = async () => {
    try {
      console.log("Starting save of all changes");
      ...
      // Process row changes, ensuring all headers are present
      if (
        pendingChanges.rows.updates.length > 0 ||
        pendingChanges.rows.newRows.length > 0
      ) {
        // Ensure all rows have values for all headers
        const completeUpdates = pendingChanges.rows.updates.map((update) => ({
          rowId: update.rowId,
          values: {
            ...Object.fromEntries(
              tableData.headers.map((header) => [header.id.toString(), ""]), // Default empty values
            ),
            ...update.values, // Overwrite with actual user input
          },
        }));

        const completeNewRows = pendingChanges.rows.newRows.map((newRow) => ({
          ...newRow,
          values: {
            ...Object.fromEntries(
              tableData.headers.map((header) => [header.id.toString(), ""]), // Default empty values
            ),
            ...newRow.values, // Overwrite with actual user input
          },
        }));

        await batchUpdate.mutateAsync({
          updates: completeUpdates.filter((update) => update.rowId > 0),
          newRows: completeNewRows,
        });
      }
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      const headersWithIds = pendingChanges.headers[tableId]?.headers ?? [];
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        const pendingRows = pendingChanges.rows.newRows.map((newRow) => ({
          id: newRow.tempId,
          tableId,
          values: {
            ...newRow.values,
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      setPendingChanges((prev) => {
        const next = { ...prev };
        delete next.headers[tableData.id];
        return next;
      });
  ...
  const handleHeaderAdd = (newHeader: { id: number; name: string }) => {
    setPendingChanges((prev) => ({
      ...prev,
      headers: {
        ...prev.headers,
        [tableData?.id ?? 0]: {
          ...
          headers: [
            ...(prev.headers[tableData?.id ?? 0]?.headers ?? []),
            newHeader,
          ],
        },
      },
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      console.log("Cell Update Started:", {
        ...
        pendingBatchState: pendingChanges.rows,
  ...
  const handleRowCreate = useCallback(() => {
    ...
    setPendingChanges((prev) => ({
      ...
      rows: {
        ...prev.rows,
        updates: [...prev.rows.updates, ...rowsNeedingUpdate],
        newRows: [
          ...prev.rows.newRows,
          {
  ...
  //     if (Object.keys(pendingChanges.headers).length === 0) return;

  //     try {
  //       headerProcessingRef.current = true;
  //       console.log("Starting header processing", {
  //         pendingHeaders: pendingChanges.headers,
  //       });
  ...
  //       console.log("Starting row processing", { pendingChanges });
