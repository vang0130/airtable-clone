...
export default function Sheet() {
  ...
  //   const selectedTable = sheetData.tables.find(t => t.id === selectedTableId);
  ...
  //     ...selectedTable,
  //     headers: [
  //       ...selectedTable.headers.filter(h =>
  ...
  //     rows: [
  //       // Existing rows with updates
  //       ...selectedTable.rows.map(row => {
  //         const pendingUpdate = pendingChanges.rows.updates.find(u => u.rowId === row.id);
  //         return pendingUpdate ? { ...row, values: pendingUpdate.values } : row;
  //       }),
  //       // New rows
  //       ...pendingChanges.rows.newRows.filter(nr => nr.tableId === selectedTableId)
  ...
  const handleHeaderAdd = (newHeader: { name: string }) => {
    ...
    // Get headers only for current table
    ...
    // Reset position counting for each table
    const maxPosition =
      currentTableHeaders.length > 0
        ? Math.max(...currentTableHeaders.map((h) => h.headerPosition))
        : 0;

    const headerWithPosition = {
      headerPosition: maxPosition + 1,
      name: newHeader.name,
      isPending: true,
    };
    ...
    updateRowsForNewHeader(headerWithPosition.headerPosition);
  };
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      if (targetRow?.id) {
        // Update existing row using ID
        setPendingChanges((prev) => ({
          ...prev,
          rows: {
            updates: [
              ...prev.rows.updates.filter(
                (u) => u.rowId !== targetRow.id && typeof u.rowId === "number",
              ),
              {
                rowId: targetRow.id!,
                values: updatedValues as Record<string, string>,
              },
            ],
            newRows: prev.rows.newRows,
          },
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Get rows only for current table
    const currentTableRows = [
      ...tableData.rows,
      ...pendingChanges.rows.newRows.filter(
        (row) => row.tableId === tableData.id,
      ),
    ].sort((a, b) => a.rowPosition - b.rowPosition);
    ...
    const nextPosition =
      currentTableRows.length > 0
        ? Math.max(...currentTableRows.map((row) => row.rowPosition)) + 1
        : 1;
    ...
    setPendingChanges((prev) => ({
      ...
      rows: {
        ...prev.rows,
        newRows: [
          ...prev.rows.newRows.filter((row) => row.tableId !== tableData.id), // Remove rows from current table
          ...prev.rows.newRows.filter((row) => row.tableId === tableData.id), // Keep rows from other tables
          newRow,
        ].sort((a, b) =>
          // Sort within same table, maintain order across different tables
          a.tableId === b.tableId
            ? a.rowPosition - b.rowPosition
            : a.tableId - b.tableId,
        ),
      },
    }));
  ...
  // Add a cleanup effect when switching tables
  useEffect(() => {
    if (selectedTableId) {
      // Reset positions when switching tables
      setTableData((prevData) => {
        if (!prevData || prevData.id === selectedTableId) return prevData;

        const newTable = sheetData?.tables.find(
          (t) => t.id === selectedTableId,
        );
        if (!newTable) return prevData;

        // Ensure positions are sequential within the new table
        return {
          ...newTable,
          headers: newTable.headers
            .sort((a, b) => a.headerPosition - b.headerPosition)
            .map((h, idx) => ({ ...h, headerPosition: idx + 1 })),
          rows: newTable.rows
            .sort((a, b) => a.rowPosition - b.rowPosition)
            .map((r, idx) => ({ ...r, rowPosition: idx + 1 })),
        };
      });
    }
  }, [selectedTableId, sheetData]);

  // Add validation to ensure positions are unique within each table
  const validatePositions = useCallback((table: Table) => {
    const headerPositions = new Set();
    ...
    table.headers.forEach((header) => {
      if (headerPositions.has(header.headerPosition)) {
        isValid = false;
        console.error(
          `Duplicate header position found: ${header.headerPosition}`,
        );
      }
      headerPositions.add(header.headerPosition);
    });

    // Check rows
    table.rows.forEach((row) => {
      if (rowPositions.has(row.rowPosition)) {
        isValid = false;
        console.error(`Duplicate row position found: ${row.rowPosition}`);
      }
      rowPositions.add(row.rowPosition);
    });
