...
export default function Sheet() {
  ...
  const columns = useMemo(() => {
    ...
    const allHeaders = [
      ...tableData.headers.filter(header =>
        !pendingChanges.headers[tableData.id]?.headers.some(
          pendingHeader => pendingHeader.headerPosition === header.headerPosition
        )
      ),
      ...(pendingChanges.headers[tableData.id]?.headers ?? [])
    ].sort((a, b) => a.headerPosition - b.headerPosition);

    return allHeaders.map(header => columnHelper.accessor(
  ...
  }, [tableData?.headers, tableData?.id, pendingChanges.headers, handleCellUpdate]);
