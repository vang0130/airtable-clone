...
export default function Sheet() {
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      if (!newHeaders?.length || !tableData?.id) return;
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        if (!old) return old;
        const currentTable = old.tables.find((t) => t.id === tableData.id);
        if (!currentTable) return old;

        console.log("Using server headers directly:", newHeaders);

        const updatedTable = {
          ...currentTable,
          // Simply use the new headers from the server
          headers: newHeaders,
          // Update rows to ensure they have all header columns
