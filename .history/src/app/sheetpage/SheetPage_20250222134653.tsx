    ...
    useEffect(() => {
      if (
        ...
        document.getElementById(
          `cell-${rowId}-${rowPosition}-${headers.headerPosition}`,
        )
      ) {
        setValue(info.getValue() ?? "");
      }
    }, [info.getValue(), rowId, rowPosition, headers.headerPosition]);

    return (
      <input
        key={`cell-${rowId}-${rowPosition}-${headers.headerPosition}`}
        id={`cell-${rowId}-${rowPosition}-${headers.headerPosition}`}
        name={`cell-${rowId}-${rowPosition}-${headers.headerPosition}`}
    ...
    try {
      ...
      const updatedPendingRows = {
        ...
        newRows: pendingChanges.rows.newRows.map((row) => ({
          tableId: row.tableId,
          values: row.values,
          rowPosition: row.position, // Map position to rowPosition
        })),
      };
      ...
      ...tableData.headers.filter(
        (header) =>
          !pendingChanges.headers[tableData.id]?.headers.some(
            (newHeader) => newHeader.position === header.headerPosition,
          ),
      ),
    ...
    setPendingChanges((prev) => ({
      ...
      rows: {
        updates: [
          ...
          const rowA = tableData.rows.find(
            (r) => r.rowPosition === a.rowPosition,
          );
          const rowB = tableData.rows.find(
            (r) => r.rowPosition === b.rowPosition,
          );
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      const isNewRow = pendingChanges.rows.newRows.some(
        (row) => row.rowPosition === rowPosition,
      );
  ...
  const columns = useMemo(() => {
    ...
    const allHeaders = Object.values(
      [
        ...
        (acc, header) => {
          acc[header.headerPosition] = header;
          return acc;
        },
  ...
  const table = useReactTable({
    data: useMemo(() => {
      const sortedRows = [...(tableData?.rows ?? [])].sort((a, b) => {
        ...
        if (a.rowPosition > 0 && b.rowPosition > 0)
