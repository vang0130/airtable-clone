    ...
    const timeoutId = setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          // Process all pending changes in one go
            ...
            if (newHeaders.length > 0) {
              ...
              utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
                ...
                return {
                  ...
                  tables: old.tables.map((table) =>
                    table.id === tableId
                      ? { ...table, headers: [...table.headers, ...newHeaders] }
                      : table,
                  ),
                };
              });
              ...
              // Update with real IDs but keep existing rows
              utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
                if (!old) return old;
                return {
                  ...old,
                      ...
                      ? {
                          ...
                          headers: table.headers.map((h) => {
                            const realHeader = createdHeaders.find(
                              (ch, idx) => newHeaders[idx]?.id === h.id,
                            );
                            return realHeader && h.id < 0 ? realHeader : h;
                          }),
                        }
                      : table,
                  ),
              ...
              console.log(
                "Header processing complete, waiting before row updates",
              );
            }
          }
