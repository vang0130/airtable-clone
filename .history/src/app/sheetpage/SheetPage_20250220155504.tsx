...
export default function Sheet() {
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      setPendingBatch((prev) => ({
        updates: [
          ...prev.updates.filter((u) => u.rowId !== rowId),
  ...
  useEffect(() => {
    ...
    // Check if cell is active or headers are processing
    if (activeCell || headerProcessingRef.current) {
      // Schedule retry if under max attempts
      if (rowRetryCountRef.current < MAX_RETRIES) {
        console.log(
          `Headers processing or cell active, scheduling row retry #${rowRetryCountRef.current + 1}`,
        );
        rowRetryRef.current = setTimeout(() => {
          rowRetryCountRef.current++;
          // Trigger effect again by creating a new object reference
          setPendingBatch((prev) => ({
    ...
    const timeoutId = setTimeout(processBatch, 2000);
    ...
    pendingBatch.newRows,
    headerProcessingRef.current,
    batchUpdate,
  ]);
