...
export default function Sheet() {
  ...
  useEffect(() => {
    ...
    // Combine current rows with updates and new rows
    const updatedRows = foundTable.rows.map((row) => {
      const pendingUpdate = pendingChanges.rows.updates.find(
        (u) => u.rowId === row.id,
      );
      return pendingUpdate
        ? { ...row, values: pendingUpdate.values }
        : { ...row, values: row.values as Record<string, string> };
    });
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // Update values ensuring all headers present
      const updatedValues = {
        ...rowValues,
        [headerId.toString()]: value,
        ...Object.fromEntries(
          allHeaders.map((_, index) => {
