...
export default function Sheet() {
  ...
  const handleHeaderAdd = async () => {
    ...
    // Update pending headers state
    setPendingHeaders((prev) => {
      const existingHeaders = prev[tableData.id]?.headers ?? [];
      const newHeader = { id: tempId, name: headerInput.trim() };

      console.log("Adding header to pending:", {
        existing: existingHeaders,
        new: newHeader,
        combined: [...existingHeaders, newHeader],
      });
      ...
      return {
        ...
        [tableData.id]: {
          ...
          headers: [...existingHeaders, newHeader],
        },
      };
    });
  ...
  const processPendingHeaders = async () => {
    try {
      ...
      if (!pendingTableHeaders.length) {
        console.log("No pending headers to process");
        return;
      }

      console.log("Processing pending headers:", {
        pendingHeaders,
        tableId: tableData?.id,
        pendingTableHeaders,
      });
      ...
      const createdHeaders = await addHeader.mutateAsync({
        tableId: tableData!.id,
        headers: pendingTableHeaders.map((h) => ({
          name: h.name,
          // Don't send the temp ID to the server
        })),
      });

      console.log("Headers created:", {
        sent: pendingTableHeaders.map((h) => h.name),
        received: createdHeaders,
      });

      if (createdHeaders?.length) {
        setTableData((prev) => {
          // Map all pending headers to their created counterparts
          const updatedHeaders = prev!.headers.map((h) => {
            // Find the matching pending header by temp ID
            const pendingIndex = pendingTableHeaders.findIndex(
              (ph) => ph.id === h.id,
            );
            if (pendingIndex >= 0 && createdHeaders[pendingIndex]) {
              // Replace temp header with created header
              return {
                id: createdHeaders[pendingIndex].id,
                name: createdHeaders[pendingIndex].name,
              };
            }
            return h;
          });
