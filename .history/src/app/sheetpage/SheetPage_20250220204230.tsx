...
export default function Sheet() {
  ...
  // Add this ref to track updates during header processing
  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate(variables) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        // Create a Map to preserve all recent changes
        const updatedRows = new Map(
          currentTable.rows.map((row) => [row.id, { ...row }]),
        );

        // Apply pending batch updates first
        pendingBatch.updates.forEach((update) => {
          const existingRow = updatedRows.get(update.rowId);
          if (existingRow) {
            updatedRows.set(update.rowId, {
              ...existingRow,
              values: {
        ...
        // Handle new rows
        [...pendingBatch.newRows, ...variables.newRows].forEach((newRow) => {
          if (!updatedRows.has(newRow.tempId)) {
            updatedRows.set(newRow.tempId, {
              id: newRow.tempId,
        ...
        console.log("Optimistic update:", {
          previousRows: currentTable.rows.map((r) => ({
            id: r.id,
            values: r.values,
          })),
          updatedRows: Array.from(updatedRows.values()).map((r) => ({
            id: r.id,
            values: r.values,
          })),
        });
        ...
        return {
          ...
          tables: old.tables.map((table) =>
            table.id === tableData?.id
              ? {
                  ...table,
                  rows: Array.from(updatedRows.values()),
                }
              : table,
          ),
        };
  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      console.log("Header mutation completed, starting updates:", {
        headerProcessing: headerProcessingRef.current,
        createdHeaders,
        pendingHeaders: pendingHeaders[tableData.id],
      });
      ...
      setTableData((prev) => {
        ...
        // Process pending cell updates with new header IDs
        const pendingUpdates = pendingCellUpdatesRef.current;
        pendingCellUpdatesRef.current = {}; // Clear the ref

        // Add pending cell updates to pendingBatch with real header IDs
        setPendingBatch((prev) => {
          const newUpdates = Object.entries(pendingUpdates).flatMap(
            ([tempHeaderId, rowUpdates]) => {
              const realHeaderId = idMapping[tempHeaderId];
              if (!realHeaderId) return [];

              return Object.entries(rowUpdates).map(([rowId, value]) => ({
          ...
          console.log("Adding pending cell updates to batch:", {
            pendingUpdates,
            newUpdates,
            currentBatch: prev,
          });
      ...
      // Clear pending headers after all updates
      ...
      console.log("Header processing completed", {
        headerProcessing: headerProcessingRef.current,
      });
    },
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // check for negative header ids in values
      const hasNegativeHeaderIds = Object.keys(rowValues).some(
        (id) => parseInt(id) < 0,
      );
      if (hasNegativeHeaderIds) {
        console.warn("Refusing update: Values contain negative header IDs", {
          values: rowValues,
      ...
      // Check for negative header IDs in tableData
      if (tableData?.headers.some((header) => header.id < 0)) {
        console.warn("Refusing update: Table contains negative header IDs", {
          headers: tableData?.headers,
        });
        return;
      }
      ...
      if (headerProcessingRef.current) {
        console.log("Headers still processing, storing update");
        // Store the update for later processing
        pendingCellUpdatesRef.current[headerId.toString()] = {
          ...pendingCellUpdatesRef.current[headerId.toString()],
          [rowId.toString()]: value,
        };

        // Show optimistic UI even during header processing
        setTableData((prev) => {
          if (!prev) return prev;
          return {
            ...prev,
      ...
      // Ensure all headers are present in the values
      const updatedValues = {
        ...rowValues,
        [headerId.toString()]: value,
        ...Object.fromEntries(
      ...
      // Always update tableData immediately for optimistic UI
      setTableData((prev) => {
        if (!prev) return prev;

        // Find any missing headers in other rows
        const rowsNeedingUpdate = prev.rows
          .filter((row) => row.id !== rowId)
          .map((row) => {
            const missingHeaders = prev.headers.filter(
            ...
            return {
              ...
              values: {
                ...row.values,
                ...Object.fromEntries(
                  missingHeaders.map((header) => [header.id.toString(), ""]),
                ),
              },
            };
          })
        ...
        // Update current row and any rows needing header updates
        const updatedRows = prev.rows.map((row) => {
          if (row.id === rowId) {
            return {
              ...row,
          ...
          const updatedRow = rowsNeedingUpdate.find((u) => u.id === row.id);
      ...
      if (headerProcessingRef.current) {
        // Store for later processing after header mutation
        pendingCellUpdatesRef.current[headerId.toString()] = {
          ...pendingCellUpdatesRef.current[headerId.toString()],
          [rowId.toString()]: value,
        };
      ...
      // Always update pendingBatch immediately
      setPendingBatch((prev) => {
        // Remove any existing updates for this row
        ...
        // Find rows that need header updates
  ...
  const handleRowCreate = useCallback(() => {
    console.log("Row creation started:", {
      headerProcessing: headerProcessingRef.current,
      headers: tableData?.headers.map((h) => ({ id: h.id, name: h.name })),
    });
    ...
    // Check for negative header IDs
    if (tableData?.headers.some((header) => header.id < 0)) {
      console.warn(
        "Refusing to create row: Table contains negative header IDs",
        {
          headers: tableData?.headers,
        },
      );
    ...
    // Verify no negative IDs in newRowValues
  ...
  // Add ref at the top with your other refs
  const deferredHeaderMutationRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (processingRef.current) {
      console.log("Row mutation in progress, deferring header processing");
      deferredHeaderMutationRef.current = true;
      return;
    }
  ...
  // ROW MUTATIONS
  useEffect(() => {
    if (pendingBatch.updates.length === 0 && pendingBatch.newRows.length === 0)
      return;
    if (processingRef.current) {
    ...
    if (headerProcessingRef.current) {
      console.log("Headers processing, deferring row processing");
      return;
    }
    if (activeCell) {
    ...
    const processBatch = async () => {
      try {
        ...
        const previousTableState = tableData;
        ...
        setTableData((prev) => {
          ...
          // Create a map of the latest values from pendingBatch
          const pendingUpdates = new Map(
            pendingBatch.updates.map((update) => [update.rowId, update.values]),
          );

          // Update rows while preserving pending changes
          const updatedRows = prev.rows.map((row) => {
            const pendingValues = pendingUpdates.get(row.id);
            if (pendingValues) {
              return {
                ...row,
                values: {
                  ...row.values,
                  ...pendingValues, // Keep the pending values
                },
              };
            }
            return row;
        ...
        // Check if we need to process deferred header mutations
        if (deferredHeaderMutationRef.current) {
          console.log(
            "Processing deferred header mutation after row completion",
          );
          deferredHeaderMutationRef.current = false;
          // Force a re-run of the header processing effect
          setPendingHeaders((current) => ({ ...current }));
        }
      }
    };
