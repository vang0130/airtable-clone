  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        const updatedTable = {
          ...currentTable,
          // Use all headers from server
          headers: newHeaders,
          // Update rows, replacing temp IDs with real IDs in chronological order
          rows: currentTable.rows.map((row) => {
            const updatedValues = {
              ...
              // Map temp IDs to real IDs maintaining chronological order
              ...Object.fromEntries(
