...
export default function Sheet() {
    ...
    useEffect(() => {
      if (
        ...
        document.getElementById(
          `cell-${info.row.original.id}-${headers.position}`,
        )
      ) {
        setValue(info.getValue() ?? "");
      }
    }, [info.getValue(), info.row.original.id, headers.position]);

    const rowId = info.row.original.id;

    return (
      <input
        key={`cell-${rowId}-${headers.position}`}
        id={`cell-${rowId}-${headers.position}`}
        name={`cell-${rowId}-${headers.position}`}
  ...
  const updateRowsForNewHeader = (headerPosition: number) => {
    ...
    // Update existing rows that aren't in pendingChanges
    const updatedExistingRows = tableData.rows
      .filter(
        (row) => !pendingChanges.rows.updates.some((u) => u.rowId === row.id),
      )
      .map((row) => ({
        rowId: row.id,
        values: {
