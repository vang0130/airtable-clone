...
export default function Sheet() {
  ...
  const handleHeaderAdd = (newHeader: { name: string }) => {
    ...
    const maxPosition = Math.max(
      ...
      ...(pendingChanges.headers[tableData.id]?.headers.map(h => h.position) ?? []),
      0
    );
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Use negative timestamp for temporary ID

    // Calculate next position
    const nextPosition =
      tableData.rows.length + (pendingChanges.rows.newRows.length ?? 0) + 1;
    ...
    setTableData((prev) => {
      ...
      return {
        ...
        rows: [
          ...prev.rows,
          { id: nextPosition, values: newRowValues, position: nextPosition },
