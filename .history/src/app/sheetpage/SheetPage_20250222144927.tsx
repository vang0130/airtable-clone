...
export default function Sheet() {
  ...
  const handleCellUpdate = useCallback(
    (rowPosition: number, headerPosition: number, value: string, rowValues: Record<string, string>) => {
      if (!tableData?.id) return;
      ...
      const targetRow = tableData.rows.find(r => r.rowPosition === rowPosition);
      ...
      if (targetRow?.id) {
        ...
        setPendingChanges(prev => ({
          ...
          rows: {
            updates: [
              ...prev.rows.updates.filter(u => u.rowId !== targetRow.id && typeof u.rowId === 'number'),
              {
                rowId: targetRow.id as number,
