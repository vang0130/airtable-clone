...
export default function Sheet() {
  ...
  const Cell = ({
    ...
    // Use a unique key for each cell's value
    const cellKey = `${info.row.original.id}-${headers.id}`;
    const [value, setValue] = useState(info.getValue() ?? "");

    // Update value when cell data changes
    useEffect(() => {
      setValue(info.getValue() ?? "");
    }, [info.getValue(), cellKey]);

    useEffect(() => {
      if (editing) {
        setActiveCell(true);
      }
    }, [editing, setActiveCell]);

    const [editing, setEditing] = useState(false);

    return (
      // Add unique key to the outer div
      <div className="w-full" key={cellKey}>
        {editing ? (
          <input
            key={`input-${cellKey}`} // Unique key for input
            value={value}
            onChange={(e) => {
          ...
          <div
            key={`display-${cellKey}`} // Unique key for display div
            onClick={() => {
              setEditing(true);
            }}
