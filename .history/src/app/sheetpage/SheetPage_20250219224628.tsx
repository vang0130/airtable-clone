...
export default function Sheet() {
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Start with empty values and validate to ensure all headers are present
    const validatedValues = validateHeaders({});

    // First update UI
    setTableData((prev) => ({
      ...prev!,
      rows: [...prev!.rows, { id: tempId, values: validatedValues }],
    }));

    // Check all existing rows for missing headers
    const rowsNeedingUpdate = tableData.rows.filter((row) => {
  ...
  }, [tableData?.id, tableData?.headers, tableData?.rows, validateHeaders]);
