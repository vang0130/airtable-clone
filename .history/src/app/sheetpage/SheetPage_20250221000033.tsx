...
export default function Sheet() {
  ...
  useEffect(() => {
    ...
    // Ensure values object only has valid header IDs
    const validHeaderIds = new Set(uniqueHeaders.map((h) => h.id.toString()));

    const cleanedRows = foundTable.rows.map((row) => ({
      ...row,
      values: Object.fromEntries(
        Object.entries(row.values).filter(([key]) => validHeaderIds.has(key)),
      ),
    }));
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      setPendingChanges((prev) => ({
        ...
        rows: {
          ...prev.rows,
          // If it's a new row, update in newRows array instead of updates
          newRows: isNewRow
