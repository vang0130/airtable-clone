...
export default function Sheet() {
  ...
  // 1. Row creation - only use real headers
  const handleRowCreate = useCallback(() => {
    if (!tableData?.id) return;
    const tempId = -Date.now();

    // Only create values for existing headers (positive IDs)
  ...
  // 2. Header creation - queue row updates after headers are created
  const addHeader = api.header.createMany.useMutation({
    onSuccess(newHeaders, variables, ctx) {
      ...
      // Queue updates for all existing rows to add the new columns
        ...
        try {
          ...
          if (pendingHeadersByTable.length > 0) {
            ...
            if (newHeaders.length > 0) {
              ...
              setPendingHeaders({}); // Clear only after successful creation
            }
          }
          ...
          if (
            ...
            await batchUpdate.mutateAsync(pendingBatch);
