  ...
  const Cell = ({
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
      if (e.key === "Tab") {
        ...
        // Important: Set both states to false BEFORE the next cell is focused
        setEditing(false);
        setActiveCell(false);

        // Let the natural tab behavior continue
        ...
        const nextClickTarget = nextCell?.querySelector('div[role="button"]');
        if (nextClickTarget instanceof HTMLElement) {
          // Use a minimal timeout to ensure state updates have processed
          setTimeout(() => {
            nextClickTarget.click();
          }, 0);
        }
      }
    };
      ...
      <div className="h-full w-full">
        {editing ? (
          ...
          <div
            ...
            className="h-full w-full cursor-text px-2"
          >
            {value}
          </div>
        )}
