  ...
  const batchUpdate = api.row.batchUpdate.useMutation({
    async onMutate(variables) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        console.log("Optimistic update result:", {
          originalRows: currentTable.rows.map((r) => ({
            id: r.id,
            values: r.values,
          })),
          updatedRows: Array.from(uniqueRows.values()).map((r) => ({
            id: r.id,
            values: r.values,
          })),
        });
    ...
    async onSuccess(result, variables, ctx) {
      ...
      console.log("Batch update success:", {
        result,
        variables,
        currentPendingBatch: pendingBatch,
      });

      // Map temporary row IDs to real IDs
      ...
      setTableData((prev) => {
        ...
        const updatedRows = prev.rows.map((row) => {
          ...
          if (realId) {
            // For new rows that got real IDs
            return {
              ...row,
              id: parseInt(realId),
            };
        ...
        console.log("Final table data update:", {
          previousRows: prev.rows.map((r) => ({ id: r.id, values: r.values })),
