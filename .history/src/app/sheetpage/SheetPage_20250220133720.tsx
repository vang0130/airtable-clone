...
export default function Sheet() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      void (async () => {
        ...
        try {
          // start processing flag to block row mutations
          ...
          const [tableIdStr, headerData] = pendingHeadersEntries[0] as [
            string,
          ...
          if (createdHeaders?.length) {
            // update table data with real IDs
            setTableData((prev) => ({
              ...prev!,
              headers: prev!.headers.map((h) => {
                const createdHeader = createdHeaders.find(
                  (_, idx) => headerData.headers[idx]?.id === h.id,
                );
                return createdHeader ?? h;
              }),
              rows: prev!.rows.map((row) => ({
                ...row,
                values: Object.entries(row.values).reduce(
                  (acc, [key, value]) => {
                    const headerIndex = headerData.headers.findIndex(
                      (h) => h.id.toString() === key,
                    );
                    const realId =
                      headerIndex >= 0
  ...
  }, [activeCell, pendingHeaders, addHeader, tableData?.id]);
  ...
  useEffect(() => {
    ...
    // to batch multiple rows
    const timeoutId = setTimeout(() => {
      void (async () => {
        // do not proceed if no rows to process
        if (
