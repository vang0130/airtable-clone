...
export default function Sheet() {
  ...
  useEffect(() => {
    if (activeCell || processingHeaders) return; // Add processingHeaders check

    const processBatchUpdates = async () => {
      if (processingRef.current || processingHeaders) {
        console.log("Waiting for processing to complete...");
        // Retry after a short delay
        setTimeout(processBatchUpdates, 500);
        return;
      }
      ...
      try {
        ...
        // Only process row updates if no headers are being processed
        if (
          !processingHeaders &&
    ...
    const timeoutId = setTimeout(processBatchUpdates, 2000);
