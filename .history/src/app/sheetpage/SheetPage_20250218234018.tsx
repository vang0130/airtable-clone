      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        const updatedTable = {
          ...
          headers: currentTable.headers.map((header) => {
            const matchingTempHeader = ctx.headersWithTempIds.find(
              (h) => h.id === header.id && h.name === header.name,
            );
            const matchingNewHeader =
              matchingTempHeader &&
              newHeaders.find(
                (h, idx) =>
                  ctx.headersWithTempIds[idx]?.id === matchingTempHeader.id &&
                  ctx.headersWithTempIds[idx]?.name === matchingTempHeader.name,
              );
