...
export default function Sheet() {
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      utils.sheet.findSheet.setData({ id: sheetId }, (old) => {
        ...
        const currentHeaderCount = currentTable.headers?.length ?? 0;
              ...
              ? {
                  ...
                  headers: [
                    ...
                    ...headers.map((h, index) => ({
                      id: -(Date.now() + index),
                      name: h.name,
                      position: currentHeaderCount + index + 1,
                    })),
                  ],
                }
              : table,
  ...
  }, [sheetData?.tables, selectedTableId, pendingChanges]);
            ...
            table.id === tableData?.id
              ? {
                  ...
                  rows: table.rows
                    .filter(
                      (row) =>
                        !variables.updates.some((u) => u.rowId === row.id),
                    ) // Remove rows being updated
                    .concat(
                      variables.updates
                        .map((update) => {
                          const existingRow = table.rows.find(
                            (r) => r.id === update.rowId,
                          );
                          if (!existingRow) return null;
                          return {
                            id: existingRow.id,
                            tableId: existingRow.tableId,
  ...
  // Make saveAllChanges a useCallback
  ...
  const handleHeaderAdd = (newHeader: { name: string }) => {
    ...
    // Calculate next position including pending headers
    const nextPosition =
    ...
    const headerWithId = {
      id: nextPosition, // Use same value for both
      position: nextPosition,
      name: newHeader.name,
    };

    // Update rows using position numbers
    const allExistingRows = [...tableData.rows, ...pendingChanges.rows.newRows];

    const updatedRows = allExistingRows.map((row) => ({
      rowId: row.id,
      values: {
        ...row.values,
        [nextPosition.toString()]: "", // Use position as key
      },
    }));
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      setPendingChanges((prev) => ({
        ...
        rows: {
          ...prev.rows,
          // If it's a new row, update in newRows array
          newRows: isNewRow
            ? prev.rows.newRows.map((row) =>
                row.id === rowId ? { ...row, values: updatedValues } : row,
              )
            : prev.rows.newRows,
          // Only add to updates if it's not a new row
          updates: isNewRow
            ? prev.rows.updates
            : [
  ...
  const columns = useMemo(() => {
    const allHeaders = [
      ...(tableData?.headers ?? []),
      ...(pendingChanges.headers[tableData?.id ?? 0]?.headers ?? []),
    ].sort((a, b) => a.position - b.position); // Sort by position, not ID

    console.log(
      "Rebuilding columns with headers:",
      allHeaders.map((h) => ({ id: h.id, name: h.name })),
    );

    return [
      ...allHeaders.map((header) => ({
        id: header.position.toString(), // Use position as the key
        header: header.name,
        accessorFn: (row: Row) => row.values[header.position.toString()],
      })),
    ];
  }, [tableData?.headers, pendingChanges.headers, tableData?.id]);

  const table = useReactTable({
    data: useMemo(() => {
      const sortedRows = [...(tableData?.rows ?? [])].sort((a, b) => {
        // Sort by position for real rows
        if (a.id > 0 && b.id > 0) return a.position - b.position;
        // Sort temp rows by their negative timestamp IDs
        if (a.id < 0 && b.id < 0) return b.id - a.id;
        // Keep real rows (positive IDs) at top
        return a.id > 0 ? -1 : 1;
      });
      return sortedRows;
    }, [tableData?.rows]),
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Calculate next position based on existing and pending rows
    const maxPosition = Math.max(
      ...tableData.rows.map(row => row.position ?? 0),
      ...pendingChanges.rows.newRows.map(row => row.position ?? 0),
      0
    );
...
export default function Sheet() {
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Only use existing headers (not pending ones) for initial values
    ...
    // Only add to newRows, not updates
    setPendingChanges((prev) => ({
      ...prev,
      rows: {
        ...prev.rows,
        newRows: [
          ...prev.rows.newRows,
          {
            ...
            values: newRowValues,
            id: tempId,
            position: tableData.rows.length + prev.rows.newRows.length + 1,
          },
        ],
        updates: prev.rows.updates, // Don't modify updates
      },
    }));
