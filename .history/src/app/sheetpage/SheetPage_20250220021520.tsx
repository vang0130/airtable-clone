...
export default function Sheet() {
  ...
  // First, verify validateHeaders is correctly checking ALL headers
  const validateHeaders = useCallback(
    (values: Record<string, string>) => {
      if (!tableData?.headers) return values;

      // This ensures ALL headers (old and new) are present
      return {
        ...values, // Keep existing values first
        ...Object.fromEntries(
          tableData.headers.map((h) => [
            h.id.toString(),
            values[h.id.toString()] ?? "", // Add missing columns with empty string
          ]),
        ),
      };
    },
  ...
  const addHeader = api.header.createMany.useMutation({
    async onMutate({ tableId, headers }) {
      ...
      const headersWithIds = headers.map((h, index) => ({
        id: generateTempId(), // Instead of -(Date.now() + index) % 1000000
        name: h.name,
        tableId,
        createdAt: new Date(),
      }));
    ...
    async onSuccess(newHeaders, variables, ctx) {
      ...
      // Just clean up pending headers state
  ...
  // Modified handleHeaderAdd for immediate processing
  const handleHeaderAdd = async () => {
    if (!tableData?.id || !headerInput.trim()) return;

    if (processingRef.current) {
      console.log("Waiting for row mutations to complete...");
      setTimeout(() => void handleHeaderAdd(), 100);
      return;
    }

    const tempId = generateTempId();
    console.log("Creating header with temp ID:", tempId);
    lastHeaderAddTime.current = Date.now();

    // Track pending header in state - preserve existing pending headers
    setPendingHeaders((prev) => {
      console.log("Current pending headers:", prev[tableData.id]?.headers);
      ...
      return {
        ...
        [tableData.id]: {
          ...
          headers: [
            ...(prev[tableData.id]?.headers ?? []),
            { id: tempId, name: headerInput.trim() },
          ],
        },
      };
    });

    // Immediate UI update with temp ID - preserve existing headers
    setTableData((prev) => {
      console.log("Current table headers:", prev?.headers);
      console.log("Adding new header to UI:", {
        id: tempId,
        name: headerInput.trim(),
      });
      ...
      return {
        ...prev!,
        headers: [...prev!.headers, { id: tempId, name: headerInput.trim() }],
    ...
    // Clear any existing timeout but preserve the pending headers
    if (headerTimeoutRef.current) {
      clearTimeout(headerTimeoutRef.current);
    }

    // Set new timeout for batched mutation
    headerTimeoutRef.current = setTimeout(() => {
      if (Date.now() - lastHeaderAddTime.current < 2000) {
        // If headers were added recently, schedule another check
        if (headerTimeoutRef.current) {
          clearTimeout(headerTimeoutRef.current);
        }
        headerTimeoutRef.current = setTimeout(() => {
  ...
  // Separate function to process pending headers
  const processPendingHeaders = async () => {
    try {
      processingRef.current = true;
      setProcessingHeaders(true);

      const pendingTableHeaders =
        pendingHeaders[tableData?.id ?? 0]?.headers ?? [];
      if (!pendingTableHeaders.length) return;

      console.log("Processing pending headers:", pendingTableHeaders);

      const createdHeaders = await addHeader.mutateAsync({
        tableId: tableData!.id,
        headers: pendingTableHeaders.map((h) => ({ name: h.name })),
      });
      ...
      if (createdHeaders?.length) {
        setTableData((prev) => ({
          ...
          headers: prev!.headers.map((h) => {
            const createdHeader = createdHeaders.find(
              (_, idx) => pendingTableHeaders[idx]?.id === h.id,
            );
            return createdHeader
              ? { id: createdHeader.id, name: createdHeader.name }
              : h;
          }),
          rows: prev!.rows.map((row) => ({
            ...row,
            values: Object.entries(row.values).reduce(
              (acc, [key, value]) => {
                const headerIndex = pendingTableHeaders.findIndex(
                  (h) => h.id.toString() === key,
                );
                const realId =
                  headerIndex >= 0
                    ? createdHeaders[headerIndex]?.id.toString()
                    : key;
                acc[realId ?? ""] = value;
                return acc;
              },
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // First update the value for this specific cell
      ...
      setPendingBatch((prev) => {
        ...
        return {
          ...
          updates: [
            ...otherUpdates,
            // Only add to updates if it's an existing row (positive ID)
            ...(rowId > 0 ? [{ rowId, values: updatedValues }] : []),
          ],
          newRows:
            rowId > 0
              ? prev.newRows
  ...
  const handleRowCreate = useCallback(() => {
    ...
    // Check each existing row individually for which headers it's missing
  ...
  useEffect(() => {
    if (activeCell || processingHeaders) return;

    const processBatchUpdates = async () => {
      if (processingRef.current || processingHeaders) {
        console.log("Already processing, skipping...");
        return;
      }

      const hasPendingHeaders = tableData?.headers.some((h) => h.id < 0);
      if (hasPendingHeaders) {
        console.log("Waiting for header mutations to complete...");
        return;
      }
      ...
      try {
        ...
        // Check all rows for missing columns
        const rowsNeedingUpdate =
          tableData?.rows
            .filter((row) => {
              // Check if this row is missing any headers
        ...
        // Combine with existing updates
        const allUpdates = [
          ...pendingBatch.updates,
          ...rowsNeedingUpdate,
        ].filter(
          (update, index, self) =>
            // Remove duplicates by rowId, keeping the latest
            index === self.findIndex((u) => u.rowId === update.rowId),
        );

        if (allUpdates.length > 0 || pendingBatch.newRows.length > 0) {
          ...
          await batchUpdate.mutateAsync({
            updates: allUpdates.filter((update) => update.rowId > 0),
            newRows: pendingBatch.newRows,
          });
    ...
    const timeoutId = setTimeout(() => void processBatchUpdates(), 2000);
