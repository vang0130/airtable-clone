  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      setTableData((prev) => {
        ...
        // Update pending batch with new header IDs
        setPendingBatch((prevBatch) => {
          const updatedUpdates = prevBatch.updates.map((update) => ({
            ...update,
            values: Object.entries(update.values).reduce(
              (acc, [key, value]) => {
          ...
          const updatedNewRows = prevBatch.newRows.map((row) => ({
            ...row,
          ...
          console.log("Updating pending batch:", {
            before: prevBatch,
            after: { updates: updatedUpdates, newRows: updatedNewRows },
          });
