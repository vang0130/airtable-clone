...
export default function Sheet() {
  ...
  // Add a ref to track the latest cell updates
  const latestCellUpdatesRef = useRef<Map<string, string>>(new Map());
            ...
            table.id === tableData?.id
              ? {
                  ...
                  rows: table.rows.map((row) => {
                    // Get all latest values for this row
                    const rowUpdates = Array.from(
                      latestCellUpdatesRef.current.entries(),
                    )
                      .filter(([key]) => key.startsWith(`${row.id}-`))
                      .reduce(
                        (acc, [key, value]) => {
                          const headerId = key.split("-")[1];
                          acc[headerId] = value;
                          return acc;
                        },
  ...
  const handleCellUpdate = useCallback(
    (
      ...
      // Store the latest value in our ref
      const cellKey = `${rowId}-${headerId}`;
      latestCellUpdatesRef.current.set(cellKey, value);
    ...
    const processBatch = async () => {
      try {
        ...
        console.log("Starting row processing", {
          pendingBatch,
          latestValues: Object.fromEntries(latestCellUpdatesRef.current),
        });
        ...
        // Reapply latest values to ensure they're visible
        setTableData((prev) => {
          if (!prev) return prev;
          return {
            ...prev,
