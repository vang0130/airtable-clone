  ...
  const addHeader = api.header.createMany.useMutation({
    ...
    async onSuccess(createdHeaders, variables, ctx) {
      ...
      console.log("Header mutation completed:", {
        createdHeaders,
        pendingHeaders: pendingHeaders[tableData.id]?.headers,
      ...
      setTableData((prev) => {
        ...
        console.log("Processing header ID mapping:", {
          processedHeaders,
          currentHeaders: prev.headers,
          idMapping: processedHeaders.map((ph, idx) => ({
            tempId: ph.id,
            realId: createdHeaders[idx]?.id,
          })),
        });
        ...
        const updatedRows = prev.rows.map((row) => ({
          ...
          values: Object.entries(row.values).reduce(
            (acc, [key, value]) => {
              ...
              console.log("Mapping cell value:", {
                rowId: row.id,
                oldKey: key,
                newKey: realId,
                value,
        ...
        console.log("Updated table data:", {
          oldHeaders: prev.headers.map((h) => h.id),
          newHeaders: updatedHeaders.map((h) => h.id),
          rowUpdates: updatedRows.map((r) => ({
            id: r.id,
            valueKeys: Object.keys(r.values),
          })),
        });
