...
export default function Sheet() {
  ...
  const handleHeaderAdd = async () => {
    ...
    try {
      processingRef.current = true; // Block row mutations
      ...
      processingRef.current = false; // Allow row mutations again
    }
  ...
  useEffect(() => {
    ...
    const processBatchUpdates = async () => {
      if (processingRef.current || processingHeaders) {
        ...
        void setTimeout(() => void processBatchUpdates(), 500);
        return;
      }

      // Only process if we have pending updates and no header processing
      ...
      try {
        ...
        // Process in a single mutation
        await batchUpdate.mutateAsync({
          updates: pendingBatch.updates.filter((update) => update.rowId > 0),
          newRows: pendingBatch.newRows,
        });

        // Clear the batch only after successful mutation
